...
export async function ensureFairyDirectories() {
  ...
  if (!isExistingFile(readme)) {
    const lines = [
      ...
      '',
      '怎么做：',
      '  1. 找一段 3–10 秒的、干净的、只有一个人的说话声（不要音乐、不要噪音）。',
      '  2. 存成 fairy_ref.wav，就放在本目录下（即和本文件同一层）。',
      '  3. 回到 DSH：设置 → Fairy → 朗读设置 → 点「重新自检」，第 3 项应该变绿。',
      '',
      '说明：',
      '  · 这个文件是「音色」。换一个文件 = 换一种音色，随时可以换。',
      '  · 文件名不必叫 fairy_ref.wav —— 直接在设置里把「参考音频路径」改成你文件的完整路径也行。',
