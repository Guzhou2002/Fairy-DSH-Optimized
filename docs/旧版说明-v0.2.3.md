# Fairy-DSH 旧版说明（v0.2.3 及之前）存档

> 下面是 README 里 **v0.2.3 时代**的原文，已整体停止维护，仅作历史参考。
> **当前版本请以根目录的 [README.md](../README.md) 为准。**
>
> 文中这些东西**在 v0.3.0 及之后已经不存在了**，照着做会找不到文件：
>
> | 文中提到 | 现状 |
> | --- | --- |
> | `install.ps1`、`install.cmd` 的数字键菜单 | 已废弃，归档在 `legacy\install-0.2.3.ps1`；现在是新的 `install.cmd`，双击即用 |
> | `uninstall.cmd` / `uninstall.ps1` | 已废弃，归档在 `legacy\`；现在用 `dsh plugin --profile web remove <包名>` |
> | `安装说明.txt` | 已废弃，归档为 `legacy\安装说明-0.2.3.txt` |
> | `build-release.ps1` | 已废弃，现在是 `pack.ps1` |
> | 两个 zip 发布包（含依赖版 / 轻量版） | 已废弃，现在分发 5 个 `.tgz` + `install.cmd` |
> | `link:` 安装方式 | 仅开发时使用，群友不需要 |
> | `patches\` / `snapshots\` / `upstream-originals\` / `lib\` | 根目录已无这些目录 |
>
> **仍然有效**的内容（兼容性说明、`dsh.bundle` 部署机制、环境要求、许可归属、语音依赖）
> 已经搬进当前 README，不必在这里找。
